...
<body>
	...
	$(function(){
   		...
   		$idInput.keyup(function(){ //키보드에 손을 때면 실행되는 함수
   			...
   			if($idInput.val().length >= 5){ // 5글자 이상되었을 때 본격적으로 중복체크
  					...
  					$.ajax({
  						...
  						success:function(result){
  							if(result > 0){ //   중복된 아이디 존재
  								idCheckValidate(1); //idCheckValidate함수에 1을 전달
  							}else{ //중복된 아이디 존재하지 않으면
  								idCheckValidate(2); //idCheckValidate함수에 2를 전달
  							}
  						},error:function(){
   				...
   				idCheckValidate(0); //idCheckValidate함수에 0을 전달
   			}
   			
   		});
   	});
    //id 체크하는 함수
	function idCheckValidate(num){ //매개변수로 숫자가 들어온다.
		...
		if(num == 0){ // 아직 중복체크를 하지 않는 경우 (아이디 적어도 5글자 이상은 되어야만 본격적으로 중복체크할 꺼임)
			...
			$("#checkResult").show();
			$("#enrollBtn").removeAttr("disabled"); //회원가입 버튼 활성화
